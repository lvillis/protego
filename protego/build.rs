...
/// This crate has a runtime dependency on artifacts produced by the `protego-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "protego-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `protego-ebpf`, so cargo will automatically rebuild us
        // if `protego-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `protego-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("protego-ebpf");
